...
on(
  ...
  async (data: { time: Date }, cb: (responseData: any) => void) => {
    ...
    if (hours > 23) { return cb({ ok: false, error: 'Hours over 23 will crash game' }); }
    if (hours > 60) { return cb({ ok: false, error: 'Minutes over 60 will crash game' }); }
    NetworkOverrideClockTime(hours, minutes, 0);

    return cb({ ok: true, hours, minutes });
  },
);
